...
CHomeKitPush::CHomeKitPush() {
	m_bLinkActive = false;
}

bool CHomeKitPush::Start()
{
	Stop();
	...
	m_thread = std::make_shared<std::thread>(&CHomeKitPush::Do_Work, this);
	SetThreadName(m_thread->native_handle(), "HomeKitPush");

	m_sConnection = m_mainworker.sOnDeviceReceived.connect(boost::bind(&CHomeKitPush::OnDeviceReceived, this, _1, _2, _3, _4));

	return (m_thread != NULL);
}

void CHomeKitPush::Stop()
{
...
void CHomeKitPush::UpdateSettings()
{
	/*
	int fActive = 0;
...
void CHomeKitPush::OnDeviceReceived(const int m_HwdID, const uint64_t DeviceRowIdx, const std::string &DeviceName, const unsigned char *pRXCommand)
{
...
namespace http {
	namespace server {
		void CWebServer::Cmd_SaveHomeKitLinkConfig(WebEmSession & session, const request& req, Json::Value &root)
		{
			/*
			if (session.rights != 2)
			{
		...
		void CWebServer::Cmd_GetHomeKitLinkConfig(WebEmSession & session, const request& req, Json::Value &root)
		{
			/*
			if (session.rights != 2)
			{
		...
		void CWebServer::Cmd_GetHomeKitLinks(WebEmSession & session, const request& req, Json::Value &root)
		{
			/*
			if (session.rights != 2)
			{
		...
		void CWebServer::Cmd_SaveHomeKitLink(WebEmSession & session, const request& req, Json::Value &root)
		{
			/*
			if (session.rights != 2)
			{
		...
		void CWebServer::Cmd_DeleteHomeKitLink(WebEmSession & session, const request& req, Json::Value &root)
		{
			/*
			if (session.rights != 2)
			{
